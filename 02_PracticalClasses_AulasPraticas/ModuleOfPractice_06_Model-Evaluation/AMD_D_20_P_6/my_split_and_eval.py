...
# EX 7
# f)
# No predicted samples -> precision
# No true samples -> recall
# g)
# 'no true samples' means there are no TRUE POSITIVE neither FALSE NEGATIVE samples
# 'no predicted samples' means there are no TRUE POSITIVE neither FALSE POSITIVE samples
# h)
# Recall metric calculates the true positive rate TP / (TP + FN)
# If there are no true samples, it will divide by zero and throws a warning
# F1-score uses the recall metric so it will throw the same warning
# i)
# Precision metric calculates the positive predicted value TP / (TP + FP)
# If there are no predicted samples, it will divide by zero and throws a warning
# F1-score uses the precision metric so it will throw the same warning
